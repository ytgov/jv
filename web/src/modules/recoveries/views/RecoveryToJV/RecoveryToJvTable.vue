...
export default {
  ...
  methods: {
    ...
    applySameDeptSelection(selection) {
      ...
      Vue.nextTick(() => {
        ...
        if (selection.value == true && selection.item.department != this.firstSelectionDept) {
          this.selectedRecoveries = this.selectedRecoveries.filter(
            (req) => req.recoveryID != selection.item.recoveryID
          );
        }
        this.loadRecoveryData();
      });
    ...
    applyAllSameDeptSelection(selection) {
      ...
      this.filteredRecoveries.forEach((rec) => {
        if (rec.department != dept) sameDepartment = false;
      });
