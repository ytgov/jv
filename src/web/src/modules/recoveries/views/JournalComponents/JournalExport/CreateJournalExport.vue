<template>
  <div>
    ...
    <v-dialog v-model="exportJournalDialog" persistent max-width="65%" scrollable>
      <v-card>
...
export default {
  ...
  methods: {
    ...
    async getJournal() {
      return axios
        .get(`${RECOVERIES_URL}/journal/${this.journalID}`)
        .then((resp) => {
    ...
    async downloadPdf() {
      ...
      return axios
        .post(`${PDF_URL}/${type}/${this.journal.journalID}`, { data }, header)
        .then(async (res) => {
          if (this.type == "Export") {
            const blob = res.data;
            const link = document.createElement("a");
...
::v-deep(td.text-start.border-dark) {
