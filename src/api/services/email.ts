...
function generateJvContent(journalID: number, senderName: string, sender:string, receiverName: string,  receiverDept: string){
...
export async function sendJvEmail(pdfBuf: Buffer | null, journalID: number, senderName: string, sender:string, receivers: string, receiverName: string,  receiverDept: string ) {
...
//_____________________
function generatePendingApprovalContent (senderName: string, sender:string, receiverName: string,  receiverDept: string){
    return `<i>Department of ${receiverDept}</i>
    <br><b>Dear ${receiverName.replace('.',' ')},</b><br><br>
    <p>
        Your recoveries request has been created and is pending your approval. 
        Please login to the 'Recoveries Application' and review the requests under 'Pending Approvals'.
        <br>
...
export async function sendPendingApprovalEmail(addReminder: boolean, senderName: string, sender:string, receivers: string, receiverName: string,  receiverDept: string ) {
    const attachments: any[] = []
    const reminder = addReminder? '(Reminder) ' : ''
    const subject = `${reminder}Recoveries Pending Approval`
    const content = generatePendingApprovalContent(senderName, sender, receiverName, receiverDept)
    return await email(attachments, receivers, subject, content).catch(console.error);
}
...
//_____________________
function generatePurchaseApprovedContent (approval: string, senderName: string, sender:string, reference: string,  receiverDept: string, comments: string){
    return `<i>Department of ${receiverDept}</i>   
    <p>
...
export async function sendPurchaseApprovedEmail(approved: boolean, senderName: string, sender:string, receivers: string, reference: string,  receiverDept: string, declineReason: string ) {
    const attachments: any[] = []
    ...
    const content = generatePurchaseApprovedContent(approval, senderName, sender, reference, receiverDept, comments)
    return await email(attachments, receivers, subject, content).catch(console.error);
}
