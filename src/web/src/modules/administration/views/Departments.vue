<template>
  <div style="width:92%; margin:0 auto;">
    ...
    <div class="mt-2">
      <v-row class="mx-1">
        <v-btn class="my-4 ml-auto" color="primary" @click="addDepartment()">
    ...
    <v-dialog v-model="departmentDialog" persistent max-width="800px" scrollable>
      <v-card>
        ...
        <v-card-text>
          ...
          <v-select label="ICT Branch" v-model="ictBranch" :items="branchList" @change="branchChanged(true)" outlined />

          <v-autocomplete label="ICT Unit" v-model="ictUnit" :items="unitList" item-text="name" outlined />
...
export default {
  ...
  methods: {
    ...
    codeFormat(code, id, len) {
      ...
      if (noErr == true && code.length == len && id < 5) document.getElementById(`gl${id + 1}`).focus();
    },
  },
};
</script>
