<template>
  <div class="mt-0 mx-6 mb-5">
    <v-dialog v-model="addNewRecoverableDialog" persistent max-width="80%" scrollable>
      <template v-slot:activator="{ on, attrs }">
        <v-btn small elevation="5" color="primary" @click="initForm()" v-bind="attrs" v-on="on">Add to JV </v-btn>
      </template>
...
export default {
  ...
  methods: {
    ...
    modifyJournal() {
      ...
      axios
        .post(`${RECOVERIES_URL}/recoverable/${this.journalID}`, body)
        .then(() => {
          this.savingData = false;
          this.$emit("updateTable");
