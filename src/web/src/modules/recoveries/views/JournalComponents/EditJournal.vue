    ...
    <v-dialog v-model="addNewJournalDialog" persistent max-width="65%">
      ...
      <v-card>
        <v-card-title class="primary" style="border-bottom: 1px solid black">
          <div class="text-h5">JV {{ journal.jvNum }} ({{ journal.status }})</div>
          <v-btn
            style="min-width: 0px"
            elevation="0"
            color="white"
            class="mt-n1 ml-auto cyan--text text--darken-4 px-2"
            @click="closeDialog"
          >
        ...
        <v-card-text>
          ...
          <v-row class="mt-15 ml-3">
            <title-card class="mr-6" titleWidth="11rem">
              ...
              <template #body>
                <div
                  style="width:20rem; min-height:2rem;"
                  :key="update"
  ...
  methods: {
    ...
    async getRecoveries() {
      return axios
        .get(`${RECOVERIES_URL}/`)
        .then((resp) => {
          this.allDeptRecoveries = resp.data.filter(
            (recovery) =>
              recovery.status == "Complete" && !recovery.journalID && recovery.department == this.journal.department
          );
        })
        .catch((e) => {
          console.log(e);
    ...
    async saveBackUPFile() {
      ...
      return await axios
        .post(`${RECOVERIES_URL}/journal-documents/${journalID}`, formData, {
    ...
    downloadDocument(docName) {
      let url = `${RECOVERIES_URL}/journal-documents/${this.journal.journalID}/${docName}`;
      window.open(url);
    },
